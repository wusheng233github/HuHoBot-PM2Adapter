    ...
    protected function handlePacket($pk) {
        ...
        switch($pktype) {
            ...
            case "cmd":
                ...
                $this->owner->response(implode("\n", $sender->getAllMessages()), $pk["header"]["id"], true);
                break;
            case "run":
            case "runAdmin":
                $this->owner->response("未实现", $pk["header"]["id"]);
                break;
            case "add":
                $this->owner->getServer()->addWhitelist($pk["body"]["xboxid"]);
                $this->owner->response("已尝试添加白名单: " . $pk["body"]["xboxid"], $pk["header"]["id"]);
                break;
            case "delete":
                $this->owner->getServer()->removeWhitelist($pk["body"]["xboxid"]);
                $this->owner->response("已尝试移除白名单: " . $pk["body"]["xboxid"], $pk["header"]["id"]);
                break;
            case "queryList":
